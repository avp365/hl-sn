...
## Запуск проекта.
 ...
 - запустить make go-srv-sn (запускает сервис srv/sn)
 ...
 Так это прототип социальной сети с использованием различных технологий, запущенные в докер окружении без CD/CD то можно опутстить тот факт, что некоторые сервисы использовуют единую кодовую базу, но разные endpoint`s, что выражается в командах: "go run ./cmd/app/main.go"   
## Мониторинг
...
## Описание сервисов
Сервис srv/sn - сервис социальной сети. Основа. Пользователи, посты, друзья.


## Описание папки report
В данной папке содержатся различные кейсы по доработке и тестированию социальной сети.